...
def find_pet(mc: MotionCommander, pid_area: PIDController1D, pid_yaw: PIDController1D):
    ...
    global state, _align_ok_since, is_pet_found

    if pix_err_ema is None and not is_pet_found:
        mc.start_linear_motion(0.3, 0.0, 0.0, 40)
        return
    if pix_err_ema is None and is_pet_found:
        return 
    if pix_err_ema is not None and not is_pet_found:
        is_pet_found = True
...
def aligning(mc: MotionCommander, pid_xy: PIDController2D):
    ...
    if (last_seen_ts is None) or (now - last_seen_ts > MISS_TIMEOUT):
        ...
        print(f'estXY: {position_estimate[0]}, {position_estimate[1]}', flush=True)
        return
...
def findpet_backhome_landing(scf):
    ...
    pid_landing = PIDController2D(
        target_point=(0.05, 0.0),
        kp=1.0, ki=0.4, kd=0.0,
...
if __name__ == '__main__':
    ...
    pet_beacon = HttpPetDetection(ip, callback=pet_detector_callback, display=True)
    pet_beacon.start()

    april_beacon = HttpAprilResolver(ip, callback=beacon_resolver_callback, display=True)
