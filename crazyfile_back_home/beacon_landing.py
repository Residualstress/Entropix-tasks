...
state = 'ALIGNING'         # 'ALIGNING' -> 'TRACK_DESCEND' -> 'FINAL_DROP' -> 'LANDED'
...
def param_deck_flow(_, value_str):
    ...
    if value:
        ...
        print('[Deck] Flow deck detected.', flush=True)
    else:
        print('[Deck] NOT detected!', flush=True)
...
def aligning(mc: MotionCommander, pid_xy: PIDController2D):
    ...
    du, dv = pix_err_ema
    ex, ey = pix_to_meters(du, dv, z)      # 等效米误差
    e_xy   = (ex**2 + ey**2)**0.5

    vx, vy = pid_xy.compute_velocity(ex, ey)
    vx = clamp(vx, -XY_SPEED_LIMIT, XY_SPEED_LIMIT)
    vy = clamp(vy, -XY_SPEED_LIMIT, XY_SPEED_LIMIT)
...
def track_descend(mc: MotionCommander, pid_xy: PIDController2D):
    ...
    # XY 误差 & 速度
    ex, ey = pix_to_meters(pix_err_ema[0], pix_err_ema[1], z)
    vx, vy = pid_xy.compute_velocity(ex, ey)
    vx = clamp(vx, -XY_SPEED_LIMIT, XY_SPEED_LIMIT)
    vy = clamp(vy, -XY_SPEED_LIMIT, XY_SPEED_LIMIT)
...
if __name__ == '__main__':
    ...
    try:
        with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
            ...
            # 检测 Flow Deck（以获取 stateEstimate）
            ...
            if not deck_attached_event.wait(timeout=5.0):
                print('No flow deck detected! Abort.', flush=True)
                sys.exit(1)
            ...
            try:
                ...
                print(f'[WARN] Arming request failed or unsupported: {e}', flush=True)

            logconf.start()

            print('[STATE] Start ALIGNING -> TRACK_DESCEND -> FINAL_DROP', flush=True)
            beacon_landing(scf)
        ...
        if hasattr(april_beacon, "stop"):
            try:
                april_beacon.stop()
            except Exception as e:
                print(f'[AprilResolver] stop() failed: {e}', flush=True)