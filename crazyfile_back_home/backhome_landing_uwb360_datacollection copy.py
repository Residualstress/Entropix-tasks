...
def save_data_to_csv(filename):
    with open(filename, 'w') as f:
        f.write('x1,y1,x2,y2\n')
        while not on_logging:
            time.sleep(0.02)
        while on_logging:
            # print(f"{uwb_position[0]},{uwb_position[1]},{position_estimate[0]},{position_estimate[1]}", flush=True)
            f.write(f"{uwb_position[0]},{uwb_position[1]},{position_estimate[0]},{position_estimate[1]}\n")
            time.sleep(0.02)
...
if __name__ == '__main__':
    ...
    # open a thread and run save_data_to_csv
    save_thread = threading.Thread(target=save_data_to_csv, args=("data_forward.csv",))
    save_thread.start()
